...
extension BlazeWidgetLayout {
    func mergedWith(_ customization: BlazeReactWidgetLayout?) -> Self {
        ...
        merged.shimmering = merged.shimmering.mergedWith(customization.shimmering)
        return merged
    }
}

extension BlazeWidgetShimmeringStyle {
    func mergedWith(_ customization: BlazeReactWidgetShimmeringStyle?) -> Self {
        guard let customization else { return self }

        var merged = self
        merged.baseColor = UIColor(hexString: customization.baseColor) ?? merged.baseColor
        merged.highlightColor = UIColor(hexString: customization.highlightColor) ?? merged.highlightColor
        return merged
    }
}
