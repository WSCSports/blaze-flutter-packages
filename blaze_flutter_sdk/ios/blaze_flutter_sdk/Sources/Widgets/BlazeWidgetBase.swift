...
class BlazeWidgetBase: NSObject, FlutterPlatformView {
    ...
    var playbackConfigurationMap: [String: AnyHashable]? {
        get {
            return creationParams["playbackConfiguration"] as? [String: AnyHashable]
        }
    }
    ...
    var tabsConfigurationMap: [String: AnyHashable]? {
        get {
    ...
    /// Per-widget delegate for the Moments "widget to tabs" fullscreen player. Built lazily and
    /// passed to the native tabs configuration by the Moments row/grid subclasses. Every callback
    /// is forwarded on this widget's own method channel (with `onMomentsContainerTabs*` method
    /// names), so each widget reports to its own Dart `momentsContainerTabsDelegate` — mirroring
    /// the per-instance `delegate`.
    lazy var containerTabsDelegate: BlazePlayerContainerTabsDelegate = .init(
        onDataLoadStarted: { [weak self] params in
            self?.onMomentsContainerTabsDataLoadStarted(playerType: params.playerType,
                                                        sourceId: params.sourceId)
        },
        onDataLoadComplete: { [weak self] params in
            self?.onMomentsContainerTabsDataLoadComplete(playerType: params.playerType,
                                                         sourceId: params.sourceId,
                                                         itemsCount: params.itemsCount,
                                                         result: params.result)
        },
        onPlayerDidAppear: { [weak self] params in
            self?.onMomentsContainerTabsPlayerDidAppear(playerType: params.playerType,
                                                        sourceId: params.sourceId)
        },
        onPlayerDidDismiss: { [weak self] params in
            self?.onMomentsContainerTabsPlayerDidDismiss(playerType: params.playerType,
                                                         sourceId: params.sourceId)
        },
        onTriggerCTA: { [weak self] params in
            self?.onMomentsContainerTabsTriggerCTA(playerType: params.playerType,
                                                   sourceId: params.sourceId,
                                                   actionType: params.actionType,
                                                   actionParam: params.actionParam) ?? false
        },
        onTriggerPlayerBodyTextLink: { [weak self] params in
            self?.onMomentsContainerTabsTriggerPlayerBodyTextLink(playerType: params.playerType,
                                                                  sourceId: params.sourceId,
                                                                  actionParam: params.actionParam) ?? .deeplink
        },
        onPlayerEventTriggered: { [weak self] params in
            self?.onMomentsContainerTabsPlayerEventTriggered(playerType: params.playerType,
                                                             sourceId: params.sourceId,
                                                             event: params.event)
        },
        onTriggerCustomActionButton: { [weak self] params in
            self?.onMomentsContainerTabsTriggerCustomActionButton(playerType: params.playerType,
                                                                  sourceId: params.sourceId,
                                                                  buttonParams: params.customActionParams)
        },
...
extension BlazeWidgetBase {
    ...
    func onTriggerCustomActionButton(playerType: BlazePlayerType, sourceId: String?, buttonParams: BlazePlayerCustomActionButtonParams) {
        sharedDelegateHandler.onTriggerCustomActionButton(
            ...
            self?.methodChannel.invokeMethod(methodName: "onTriggerCustomActionButton", encodable: params)
        }
    }
...
extension BlazeWidgetBase {
    ...
    func onMomentsContainerTabsDataLoadStarted(playerType: BlazePlayerType, sourceId: String?) {
        sharedDelegateHandler.onDataLoadStarted(
            playerType: playerType,
            sourceId: sourceId
        ) { [weak self] params in
            self?.methodChannel.invokeMethod(methodName: "onMomentsContainerTabsDataLoadStarted", encodable: params)
        }
    }

    func onMomentsContainerTabsDataLoadComplete(playerType: BlazePlayerType,
                                                sourceId: String?,
                                                itemsCount: Int,
                                                result: BlazeResult) {
        sharedDelegateHandler.onDataLoadComplete(
            ...
            self?.methodChannel.invokeMethod(methodName: "onMomentsContainerTabsDataLoadComplete", encodable: params)
        }
    }

    func onMomentsContainerTabsPlayerDidAppear(playerType: BlazePlayerType, sourceId: String?) {
        sharedDelegateHandler.onPlayerDidAppear(
            playerType: playerType,
            sourceId: sourceId
        ) { [weak self] params in
            self?.methodChannel.invokeMethod(methodName: "onMomentsContainerTabsPlayerDidAppear", encodable: params)
        }
    }

    func onMomentsContainerTabsPlayerDidDismiss(playerType: BlazePlayerType, sourceId: String?) {
        sharedDelegateHandler.onPlayerDidDismiss(
            playerType: playerType,
            sourceId: sourceId
        ) { [weak self] params in
            self?.methodChannel.invokeMethod(methodName: "onMomentsContainerTabsPlayerDidDismiss", encodable: params)
        }
    }

    func onMomentsContainerTabsTriggerCTA(playerType: BlazePlayerType,
                                          sourceId: String?,
                                          actionType: BlazeCTAActionType,
                                          actionParam: String) -> Bool {
        return sharedDelegateHandler.onTriggerCTA(
            ...
            self?.methodChannel.invokeMethod(methodName: "onMomentsContainerTabsTriggerCTA", encodable: params)
        }
    }

    func onMomentsContainerTabsTriggerPlayerBodyTextLink(playerType: BlazePlayerType, sourceId: String?, actionParam: String) -> BlazeLinkActionHandleType {
        return sharedDelegateHandler.onTriggerPlayerBodyTextLink(
            playerType: playerType,
            sourceId: sourceId,
            actionParam: actionParam
        ) { [weak self] params in
            self?.methodChannel.invokeMethod(methodName: "onMomentsContainerTabsTriggerPlayerBodyTextLink", encodable: params)
        }
    }

    func onMomentsContainerTabsPlayerEventTriggered(playerType: BlazePlayerType, sourceId: String?, event: BlazePlayerEvent) {
        sharedDelegateHandler.onPlayerEventTriggered(
            playerType: playerType,
            sourceId: sourceId,
            event: event
        ) { [weak self] params in
            self?.methodChannel.invokeMethod(methodName: "onMomentsContainerTabsPlayerEventTriggered", encodable: params)
        }
    }

    func onMomentsContainerTabsTriggerCustomActionButton(playerType: BlazePlayerType, sourceId: String?, buttonParams: BlazePlayerCustomActionButtonParams) {
        sharedDelegateHandler.onTriggerCustomActionButton(
            playerType: playerType,
            sourceId: sourceId,
            buttonParams: buttonParams
        ) { [weak self] params in
            self?.methodChannel.invokeMethod(methodName: "onMomentsContainerTabsTriggerCustomActionButton", encodable: params)
        }
    }

    func onMomentsContainerTabsTabSelected(playerType: BlazePlayerType, sourceId: String?, tabIndex: Int) {
